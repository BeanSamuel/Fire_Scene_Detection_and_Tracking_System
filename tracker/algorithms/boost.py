...
from tracker.settings import GeneralSettings, BoostTrackSettings, BoostTrackPlusPlusSettings
from tracker.utils.association import (
    associate, iou_batch, MhDist_similarity, shape_similarity, soft_biou_batch
)
from tracker.algorithms.ecc import ECC
from tracker.algorithms.kalman import KalmanFilter

def convert_bbox_to_z(bbox):
    """
    Convert bbox [x1,y1,x2,y2] -> z [center_x, center_y, height, aspect_ratio]
    """
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1]
    x = bbox[0] + w / 2.0
...
def convert_x_to_bbox(x, score=None):
    """
    Convert state [center_x, center_y, height, aspect_ratio] -> bbox [x1,y1,x2,y2]
    """
    h = x[2]
    r = x[3]
    w = 0 if r <= 0 else r * h
...
class BoostTrack(object):
    def __init__(self, video_name: Optional[str] = None):
        ...
        self.iou_threshold = GeneralSettings.get('iou_threshold')
        self.det_thresh = GeneralSettings.get('det_thresh')
        self.min_hits = GeneralSettings.get('min_hits')

        self.lambda_iou = BoostTrackSettings.get('lambda_iou')
        self.lambda_mhd = BoostTrackSettings.get('lambda_mhd')
        self.lambda_shape = BoostTrackSettings.get('lambda_shape')
        self.use_dlo_boost = BoostTrackSettings.get('use_dlo_boost')
        self.use_duo_boost = BoostTrackSettings.get('use_duo_boost')
        self.dlo_boost_coef = BoostTrackSettings.get('dlo_boost_coef')

        self.use_rich_s = BoostTrackPlusPlusSettings.get('use_rich_s')
        self.use_sb = BoostTrackPlusPlusSettings.get('use_sb')
        self.use_vt = BoostTrackPlusPlusSettings.get('use_vt')

        if GeneralSettings.get('use_ecc'):
    ...
    def update(self, dets, img_tensor, img_numpy, tag):
        ...
        img_tensor: (B, C, H, W) tensor (for scale reference)
        img_numpy: 原始 numpy 圖像 (H, W, 3)
        tag: video/frame標識字串
        回傳: [[x1,y1,x2,y2,ID,confidence], ...]
        """
        ...
        # Rescale dets to match img_numpy shape
