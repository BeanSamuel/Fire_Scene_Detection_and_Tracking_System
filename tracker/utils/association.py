...
def shape_similarity(detects: np.ndarray, tracks: np.ndarray) -> np.ndarray:
    from tracker.settings import BoostTrackSettings
    if not BoostTrackSettings.get('s_sim_corr'):
...
def associate(
    ...
    return linear_assignment(detections, trackers, iou_matrix, cost_matrix, iou_threshold)